...
\begin{align}
	...
	\sum_{i} B_{ij} & = \sum_{i} A_{ij} - \frac{d_j}{2m}\sum_{i} d_i = d_j^{out} - d_j^{in},
\end{align}
...
\begin{align}
	\frac{1}{2}(1 + s_i s_j) & =
	\begin{cases}
		0 \:\text{if} \:(i,j) \:\text{vertices are in different groups} \\
		1 \:\text{if} \:(i,j) \:\text{vertices are in the same group}.
	\end{cases}
\end{align}
