...
\begin{align}
	{x_k} = A^k x_0\approx \lambda_1^k(\text{component along $v_1$}) + \lambda_2^k (\text{component along $v_2$}) + \dots.
\end{align}
...
\begin{align}
	...
	2 \sum_j L_{ij}s_j - 2 \alpha s_i - 2 \beta & = 0  \\
	L s - \alpha s - \beta \bm{1}               & = 0.
\end{align}
Using the left eigenvector of the Laplacian, we can solve for $\beta$
\begin{align}
	\bm{1}^T L s - \alpha \bm{1}^T s - \beta \bm{1}^T\bm{1} & =                          \\
	0 - \alpha (n_1 - n_2) - \beta n                        & = 0                        \\
	\beta                                                   & = \alpha\frac{n_2-n_1}{n}.
\end{align}
We can now substitute for $\beta$
\begin{align}
	L s & - \alpha s -  \alpha\frac{n_2-n_1}{n}\bm{1} = 0        \\
	L s & = \alpha \left( s + \frac{n_2 - n_1}{n}\bm{1} \right).
\end{align}
The Laplacian has the following property $L \bm{1} = 0$ so we can write
\begin{align}
	L\left( s + \frac{n_2 - n_1}{n}\bm{1} \right) & = \alpha \left( s + \frac{n_2 - n_1}{n}\bm{1} \right).
\end{align}
This allows us to define a new vector $x$
\begin{align}
	x   & = s + \frac{n_2 - n_1}{n} \bm{1} \\
	L x & = \alpha x.
\end{align}
...
\begin{align}
	R & = \frac{1}{4} s^T L s = \frac{1}{4}x^T L x = \frac{1}{4} \alpha x^T x,
\end{align}
since $s$ and $x$ by $\frac{n_2 - n_1}{n}\bm{1}$ which is in kernel of $L$.

Then
\begin{align}
	x^T x & = \left(s + \frac{n_2 - n_1}{n} \bm{1}\right) ^T \left(s + \frac{n_2 - n_1}{n} \bm{1}\right)                            \\
	x^T x & = s^T s + \frac{n_2 - n_1}{n} \left(s^T \bm{1} + \bm{1}^T s\right) + \left(\frac{n_2 - n_1}{n}\right)^2 \bm{1}^T \bm{1} \\
	s^s   & = n, \: s^T \bm{1} + \bm{1}^Ts = 2(n_1 - n_2), \: \bm{1}^T \bm{1} = n                                                   \\
	x^T x & = n -2 \frac{(n_2-n_1)^2}{n} + \left(\frac{n_2-n_1}{n}\right)^2 n                                                       \\
	x^T x & = n -2 \frac{(n_2-n_1)^2}{n} + \frac{(n_2-n_1)^2}{n}                                                                    \\
	x^Tx  & = n - \frac{(n_2-n_1)^2}{n} = \frac{n^2 - (n_2-n_1)^2}{n}                                                               \\
	x^Tx  & = \frac{(n_2 + n_1)^2 - (n_2-n_1)^2}{n} = \frac{n_2^2 + 2 n_1 n_2 + n_1^2 - n_2^2 + 2n_1n_2 - n_1^2}{n}                 \\
	x^Tx  & = \frac{4n_1n_2}{n}.
\end{align}

Finally, the cut set size is
\begin{align}
	R & = \frac{1}{4} \alpha \frac{4n_1n_2}{n} = \alpha \frac{n_1n_2}{n}.
\end{align}

To minimize the size of the cut set, we need to find the eigenvector corresponding to the minimal eigenvalue,
but also
\begin{align}
	\bm{1}^T x & = \bm{1}^T s + \frac{n_2 - n_1}{n} \bm{1}^T\bm{1} = (n_1-n_2) + (n_2 - n_1) = 0.
\end{align}
Choosing the zero eigenvalue of the Laplacian, the corresponding eigenvector is $\bm{1}$, but then
\begin{align}
	\bm{1}^T x & = \bm{1}^T \bm{1} = n \neq 0.
\end{align}
...
However, after you find the optimum, you need to find actual restricted values of $s_i = \pm 1$ closest to the nonrestricted optimum $s = x - \frac{n_2-n_1}{n}\bm{1}$, that is
\begin{align}
	{\rm maximize} \quad & s^T \left(x - \frac{n_2 - n_1}{n} \bm{1}\right) = \sum_i s_i \left(x_i - \frac{n_2 - n_1}{n}\right).
\end{align}
