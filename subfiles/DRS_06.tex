...
It is a global measure, which says that if a vertex is connected to two neighbors what is the probability that these two neighbors are also connected.
\begin{definition}{Transitivity}{}
	...
	\end{align}
	A connected triple is an unordered path of length 2 so $A-B-C$ is the same as $C-B-A$, but $B-A-C$ is different.\\
	A single triangle can be written as 3 connected triples, one centered at each node:
	\begin{itemize}
		\item $A-B-C$
		\item $A-C-B$
		\item $B-A-C$.
	\end{itemize}
	A triangle is essentially a closed triple.
	The formula is then: how many closed triples among all triples.
\end{definition}

\section{Define local clustering.}
Local clustering is a local variant of the transitivity.
\begin{align}
...
\end{definition}
\begin{definition}{Cluesterable network}{}
...
\end{definition}
\begin{theorem}{Harrary's therorem}{}
	A structurally balanced network is certainly clusterable.
...
\end{itemize}
The notion of similarity is crucial for hierarchical clustering.
...
\begin{align}
	\sigma'_{ij} & = \alpha \sum_{kl} A_{ik}A_{jl}\sigma_{kl} = \alpha \sum_{kl}A_{ik}\sigma_{kl}A_{lj}^T \\
...
\begin{align}
	% \sum_{i,j} \delta(c_i, c_j) = 
	\frac{1}{2}\sum_{i,j} A_{ij}\delta(c_i, c_j)
\end{align}
...
The difference is
\begin{align}
	\frac{1}{2}\sum_{i,j} A_{ij}\delta(c_i, c_j) - \frac{1}{2} \sum_{i,j} \frac{d_i d_j}{2m} \delta (c_i, c_j).
\end{align}
...
\begin{align}
	...
	{\rm cov}(x_i, x_j) & = \frac{1}{2m} \sum_{i,j}\left(A_{ij} - \frac{d_i d_j}{2m}\right)x_i x_j.
\end{align}
