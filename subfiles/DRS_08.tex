...
The adjacency list has storage complexity $O(n + m)$ which is less then $O(n^2)$ for sparse networks.
...
A node stores all its neighbors in a tree structure. For every node, there is a unique tree. The edges in the tree do not represent edges in the network.
