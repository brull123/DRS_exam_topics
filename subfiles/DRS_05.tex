...
\begin{definition}{PageRank}{}
	...
	\begin{align}
		...
		x                       & = \alpha A D^{out-1} x + \bm{1}\beta                           \\
		(I - \alpha A D^{out-1}) x & = \bm{1}\beta                                                  \\
		x                       & = (I - \alpha A D^{out-1})^{-1} \bm{1} \beta                   \\
		x                       & = \left((D^{out} - \alpha A)D^{out-1}\right)^{-1} \bm{1} \beta\\
		x                       & = D^{out}\left(D^{out} - \alpha A\right)^{-1}\bm{1}\beta       \\
		D^{out}                 & = {\rm diag}(\max{(d_i^{out}, 1)})
	\end{align}
\end{definition}

\subsection{Modified PageRank}
\begin{definition}{Modified PageRank}{}
	\begin{align}
		x_i     & = \alpha \sum_j A_{ij} \frac{x_j}{d_j^{out}} + \beta_i   \\
		x       & = D^{out}\left(D^{out} - \alpha A\right)^{-1}\bm{\bar{\beta}} \\
		D^{out} & = {\rm diag}(\max{(d_i^{out}, 1)})
	\end{align}
\end{definition}
...
\begin{definition}{Authories and Hubs}{}
	...
	In vector form
	\begin{align}
		x      & = \alpha A y,            & \quad y       & = \beta A^T x           \\
		x      & = \alpha A  \beta A^T x, & \quad y       & = \beta A^T \alpha A y  \\
		AA^T x & = (\alpha \beta)^{-1} x, & \quad A^T A y & = (\beta \alpha)^{-1}y.
	\end{align}
	Meaning that the resulting hub and authority centralities are given by the eigenvectors of matrices $AA^T$ and $A^TA$.
\end{definition}
...
\begin{definition}{$k$-plex}
	A $k$-plex (of size $n$) is a maximal subset of $n$ vertices such that each one member is connected to at least $n-k$ others in that subset.
\end{definition}
\subsection{k-core}
...
\end{definition}
\begin{itemize}
	\item A 1-component is a regular component
\end{itemize}
\end{document}